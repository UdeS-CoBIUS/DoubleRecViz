...
def computeStats():
	sameTreeSHT_LT_l = []
	SHT_diff_LT_same_Rec_l = []
	SHT_Best_LT_l = []
	LT_Best_SHT_l = []

	sameTreeSHT_LTBD_l = []
	SHT_diff_LTBD_same_Rec_l = []
	SHT_Best_LTBD_l = []
	LT_BD_Best_SHT_l = []
			
	sameTreeSHT_ISOSEL_l = []
	SHT_diff_ISOSEL_same_Rec_l = []
	SHT_Best_ISOSEL_l = []
	ISOSEL_Best_SHT_l = []
	...
	for e in  listes:
		...
		result = open("statistics/statistics_" + sub_path + ".csv", "w")
		result.write("geneTreeID \t  nbLeaves  \t  NbSharedLeaves \t AU_TreeBest  \t AU_Ensembl \t AU_ReconstructEnsembl  \t AU_Isosel \t TreeBestPassTest   \t EnsemblTreePassTest \t ReconstructEnsemblTreePassTest \t au_isosel_pass_test \t TreeBestBetter   \t EnsemblBetter \t ReconstructEnsemblBetter \t REC_TreeBest  \t  REC_Ensembl \t REC_ReconstructEnsembl \t RecIsosel  \t Treebest_REC_is_best	 \t Ensembl_REC_is_best \tEnsembl_REC_reconstruct_is_best  \t Isosel_REC_is_best \n")
		geneFamilyIds = []
		sameTreeSHT_LT = 0
		SHT_diff_LT_same_Rec = 0
		SHT_Best_LT = 0
		LT_Best_SHT = 0

		sameTreeSHT_LTBD = 0
		SHT_diff_LTBD_same_Rec = 0
		SHT_Best_LTBD = 0
		LT_BD_Best_SHT = 0
				
		sameTreeSHT_ISOSEL = 0
		SHT_diff_ISOSEL_same_Rec = 0
		SHT_Best_ISOSEL = 0
		ISOSEL_Best_SHT = 0
				
		for root, dirs, files in os.walk("ensemblGeneTree/sequences_" + sub_path + ""): 
			
				for file in files:
					tmp = file.split("_")
					if file.count("_")>0 and len(tmp) > 0 :
						geneFamilyId = tmp[0]
						if not(geneFamilyId in geneFamilyIds) and os.path.exists("ensemblGeneTree/sequences_" + sub_path + "/" + geneFamilyId + "_stats.txt") == True:
							geneFamilyIds.append(geneFamilyId)

		for geneFamilyId in geneFamilyIds:
			try:
				print(geneFamilyId)
				sharedLeavesFile = open("ensemblGeneTree/sequences_" + sub_path + "/" + geneFamilyId + "_stats.txt", "r")
				...
				tree_lines = []
				newickfile = open("ensemblGeneTree/trees_" + sub_path + "/" + geneFamilyId + ".nw", "r")
				...
				if nb_input_trees!=5:
					...
					orthoGroup_result_file_consel_str = "ensemblGeneTree/sequences_" + sub_path + "/" + geneFamilyId + "_Aln_ByIsoSel.phy._seq_ByIsoSel_consel"
					#orthoGroup_result_file_consel_str = "ensemblGeneTree/sequences_" + sub_path + "/" + geneFamilyId + "_Aln_ByOrthoGroup.phy._seq_OrthoGroup_consel"
					#orthoGroup_result_file_consel_str =  "ensemblGeneTree/sequences_" + sub_path + "/" + geneFamilyId + "_Aln_ByEnsemblGeneTree.phy._seq_Ensembl_consel"
					ensembl_result_file_consel_str = "ensemblGeneTree/sequences_" + sub_path + "/" + geneFamilyId + "_Aln_ByEnsemblGeneTree.phy._seq_Ensembl_consel"
					...
					if os.path.exists(orthoGroup_result_file_consel_str) : #and os.path.exists(ensembl_result_file_consel_str):
						...
						if len(lines_orthoGroup_sequences) - 5 == nb_input_trees:
							...
							line = lines_orthoGroup_sequences[i_isosel]
							parts = line.split()
							au_isosel = float(parts[4])
							...
							if au_isosel >= au_value_treshold:
								au_isosel_pass_test = 1
							...
							au_isosel_is_better = 0
					
							if au_treeBest > au_ensembl and au_treeBest>au_reconstruct_ensembl and au_treeBest > au_isosel:
								au_treebest_is_better = 1
							elif au_ensembl > au_treeBest and au_ensembl>au_reconstruct_ensembl and au_ensembl>au_isosel:
								au_ensembl_is_better = 1
							elif au_reconstruct_ensembl > au_ensembl and au_reconstruct_ensembl>au_treeBest and au_reconstruct_ensembl>au_isosel:
								au_reconstruct_ensembl_is_better = 1	
							elif au_isosel > au_ensembl and au_isosel>au_treeBest and au_isosel>au_reconstruct_ensembl:
								au_isosel_is_better = 1						

							reconciliation_score1 =0
							reconciliation_score2 =0
							reconciliation_score3 =0
							reconciliation_score4 =0
							"""
							#i_best_ensembl -= 1
							...
							for node in treeFromIsosel.traverse("postorder"):
								if node.is_leaf():
									#node.name =  dictSpe[mapping_gene_species[node.name]] + "_" + node.name
									node.name =  dictSpe[mapping_gene_species[dictIsosel[node.name]]] + "_" + node.name
							recon_tree4, events4 = treeFromIsosel.reconcile(sptree)
							nbDup4 = numberOfDuplicationGtoS(recon_tree4)
							nbLost4 = numberOfLostGtoS(recon_tree4)
							reconciliation_score4 = nbDup4 + nbLost4
							...
							print(tree_lines[0] == tree_lines[2], len(tree_lines))
							print(reconciliation_score1, reconciliation_score3, reconciliation_score1 == reconciliation_score3)
							if (tree_lines[0] == tree_lines[2]):						
								sameTreeSHT_LT += 1
							elif (reconciliation_score1 == reconciliation_score3):
								SHT_diff_LT_same_Rec += 1
							elif (reconciliation_score1 < reconciliation_score3):
								SHT_Best_LT += 1
							else:
								LT_Best_SHT += 1
							nb_t +=1
							print("_____________________________",sameTreeSHT_LT, SHT_diff_LT_same_Rec, SHT_Best_LT, LT_Best_SHT, nb_t)
							if (tree_lines[0] == tree_lines[3]):						
								sameTreeSHT_LTBD += 1
							else:
								if (reconciliation_score1 == reconciliation_score2):
									SHT_diff_LTBD_same_Rec += 1
								else:
									if (reconciliation_score1 < reconciliation_score2):
										SHT_Best_LTBD += 1
									else:
							...
							if (tree_lines[0] == tree_lines[4]):						
								sameTreeSHT_ISOSEL += 1
							else:
								if (reconciliation_score1 == reconciliation_score4):
									SHT_diff_ISOSEL_same_Rec += 1
								else:
									if (reconciliation_score1 < reconciliation_score4):
										SHT_Best_ISOSEL += 1
									else:
							...
							reconciliation_score4_is_better = 0					
							

							if reconciliation_score1 < reconciliation_score2 and reconciliation_score1<reconciliation_score3 and reconciliation_score1<reconciliation_score4:
								reconciliation_score1_is_better = 1
							elif reconciliation_score2 < reconciliation_score1 and reconciliation_score2<reconciliation_score3 and reconciliation_score2<reconciliation_score4:
								reconciliation_score2_is_better = 1
							elif reconciliation_score3 < reconciliation_score1 and reconciliation_score3<reconciliation_score2 and reconciliation_score3<reconciliation_score4:
								reconciliation_score3_is_better = 1
							elif reconciliation_score4 < reconciliation_score1 and reconciliation_score4<reconciliation_score2 and reconciliation_score4<reconciliation_score3:
								reconciliation_score4_is_better = 1


							print("writing result...")
							result.write(geneFamilyId + "\t" + str(nb_leaves) +  "\t" + str(percentageSharedLeaves) +  "\t" + str(au_treeBest) + "\t" + str(au_ensembl) + "\t" + str(au_reconstruct_ensembl) + "\t" + str(au_isosel) + "\t" 
								+	str(au_treeBest_pass_test)  + "\t" + str(au_ensembl_pass_test) + "\t" + str(au_reconstruct_ensembl_pass_test) + "\t" + str(au_isosel_pass_test)  + "\t"  +  str(au_treebest_is_better) 
								+ "\t" + str(au_ensembl_is_better)  + "\t" + str(au_reconstruct_ensembl_is_better)  + "\t"+ 
								str(reconciliation_score1) + "\t" + str(reconciliation_score2) + "\t" +  str(reconciliation_score3) + "\t"  +  str(reconciliation_score4) + "\t" + str(reconciliation_score1_is_better)  + "\t" + str(reconciliation_score2_is_better)  + "\t"+ 	str(reconciliation_score3_is_better)  + "\t"+ 	str(reconciliation_score4_is_better) + "\n")					
					
						#else:
			except:
				pass						
		result.close()	
		sameTreeSHT_LT_l.append(sameTreeSHT_LT*100.0/nb_t)
		SHT_diff_LT_same_Rec_l.append(SHT_diff_LT_same_Rec*100.0/nb_t)
		SHT_Best_LT_l.append(SHT_Best_LT*100.0/nb_t)
		LT_Best_SHT_l.append(LT_Best_SHT*100.0/nb_t)

		sameTreeSHT_LTBD_l.append(sameTreeSHT_LTBD*100.0/nb_t)
		SHT_diff_LTBD_same_Rec_l.append(SHT_diff_LTBD_same_Rec*100.0/nb_t)
		SHT_Best_LTBD_l.append(SHT_Best_LTBD*100.0/nb_t)
		LT_BD_Best_SHT_l.append(LT_BD_Best_SHT*100.0/nb_t)
				
		sameTreeSHT_ISOSEL_l.append(sameTreeSHT_ISOSEL*100.0/nb_t)
		SHT_diff_ISOSEL_same_Rec_l.append(SHT_diff_ISOSEL_same_Rec*100.0/nb_t)
		SHT_Best_ISOSEL_l.append(SHT_Best_ISOSEL*100.0/nb_t)
		ISOSEL_Best_SHT_l.append(ISOSEL_Best_SHT*100.0/nb_t)
		print(sameTreeSHT_LT_l)
		print(SHT_diff_LT_same_Rec_l)
		print(SHT_Best_LT_l)
		print(LT_Best_SHT_l)

		print(sameTreeSHT_LTBD_l)
		print(SHT_diff_LTBD_same_Rec_l)
		print(SHT_Best_LTBD_l)
		print(LT_BD_Best_SHT_l)
				
		print(sameTreeSHT_ISOSEL_l)
		print(SHT_diff_ISOSEL_same_Rec_l)
		print(SHT_Best_ISOSEL_l)
		print(ISOSEL_Best_SHT_l)	
