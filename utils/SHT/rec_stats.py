...
def computeStats():
	...
	LT_eq_SHT = 0.0
	LT_diff_SHT_but_same_rec = 0.0
	LT_Better_SHT = 0.0
	SHT_Better_LT = 0.0

	LTBD_eq_SHT = 0.0
	LTBD_diff_SHT_but_same_rec = 0.0
	LTBD_Better_SHT = 0.0
	SHT_Better_LTBD = 0.0

	compteur = 0

	#result = open("statistics/statistics_40_60.csv", "w")
	...
	for root, dirs, files in os.walk("ensemblGeneTree/sequences_40_60"): 
			...
			for file in files:
				...
				if file.count("_")>0 and len(tmp) > 0 :
					geneFamilyId = tmp[0]
					if not(geneFamilyId in geneFamilyIds) and os.path.exists("ensemblGeneTree/sequences_40_60/" + geneFamilyId + "_stats.txt") == True:
						geneFamilyIds.append(geneFamilyId)

	for geneFamilyId in geneFamilyIds:
		print(geneFamilyId)
		sharedLeavesFile = open("ensemblGeneTree/sequences_40_60/" + geneFamilyId + "_stats.txt", "r")
		...
		tree_lines = []
		newickfile = open("ensemblGeneTree/trees_40_60/" + geneFamilyId + ".nw", "r")
		...
		if nb_input_trees!=4:
			...
			orthoGroup_result_file_consel_str = "ensemblGeneTree/sequences_40_60/" + geneFamilyId + "_Aln_ByOrthoGroup.phy._seq_OrthoGroup_consel"
			#orthoGroup_result_file_consel_str =  "ensemblGeneTree/sequences_40_60/" + geneFamilyId + "_Aln_ByEnsemblGeneTree.phy._seq_Ensembl_consel"
			ensembl_result_file_consel_str = "ensemblGeneTree/sequences_40_60/" + geneFamilyId + "_Aln_ByEnsemblGeneTree.phy._seq_Ensembl_consel"
			...
			if os.path.exists(orthoGroup_result_file_consel_str) : #and os.path.exists(ensembl_result_file_consel_str):
				...
				if len(lines_orthoGroup_sequences) - 5 == nb_input_trees:
					...
					rf1, max_rf1, common_leaves1, parts_t11, parts_t21, v51, v61 = treeFromEnsembl.robinson_foulds(treeFromtreeBest)
					rf2, max_rf2, common_leaves2, parts_t12, parts_t22, v52, v62 = treeFromReconstructEnsembl.robinson_foulds(treeFromtreeBest)
					...
					if rf1 == 0:
						...
						if reconciliation_score1 == reconciliation_score2:
							LT_diff_SHT_but_same_rec += 1
						else:
							if reconciliation_score1 < reconciliation_score2:
								SHT_Better_LT +=1
							elif reconciliation_score1 > reconciliation_score2:
								LT_Better_SHT += 1
					...
					if rf2 == 0:
						...
						if reconciliation_score1 == reconciliation_score3:
							LTBD_diff_SHT_but_same_rec +=1 
						else:
							if reconciliation_score1 < reconciliation_score3:
								SHT_Better_LTBD +=1
			...
			#print(compteur)
			#print("LT_eq_SHT ", LT_eq_SHT*100/compteur,	"LT_diff_SHT_but_same_rec", LT_diff_SHT_but_same_rec*100/compteur,	"LT_Better_SHT", LT_Better_SHT*100/compteur,	"SHT_Better_LT", SHT_Better_LT*100/compteur, "LTBD_eq_SHT", LTBD_eq_SHT*100/compteur,	"LTBD_diff_SHT_but_same_rec", LTBD_diff_SHT_but_same_rec*100/compteur,	"LTBD_Better_SHT", LTBD_Better_SHT*100/compteur,	"SHT_Better_LTBD", SHT_Better_LTBD*100/compteur)					
	print("LT_eq_SHT ", LT_eq_SHT*100/compteur,	"LT_diff_SHT_but_same_rec", LT_diff_SHT_but_same_rec*100/compteur,	"LT_Better_SHT", LT_Better_SHT*100/compteur,	"SHT_Better_LT", SHT_Better_LT*100/compteur, "LTBD_eq_SHT", LTBD_eq_SHT*100/compteur,	"LTBD_diff_SHT_but_same_rec", LTBD_diff_SHT_but_same_rec*100/compteur,	"LTBD_Better_SHT", LTBD_Better_SHT*100/compteur,	"SHT_Better_LTBD", SHT_Better_LTBD*100/compteur)
	#result.close()	
	"""
	result.write("\n\n\n")
...
def sharedLeaves():
    ...
    for e in  listes:
        ...
        #moyenne shareleaves
	    #bars1.append(np.mean(percentageSharedLeaves))
	    all_list.append(percentageSharedLeaves)

        # Choose the height of the error bars (bars1)
        #ecart type share leaf
	    #yer1.append(np.std(percentageSharedLeaves))
