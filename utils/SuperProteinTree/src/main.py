...
def main():
    ...
    if len(Q.keys()) == 1:
        ...
        i = 1
        result.write("Super Proteine Tree : \t \t " +   "\n" + " " + str(len(spanTree[Q.keys()[0]].get_leaf_names())))

        #result.write(spanTree[Q.keys()[0]])

        result.write("TREE IN NEWICK FORMAT : \n")
        result.write(spanTree[Q.keys()[0]].write(format=9))
        outputfile.write(spanTree[Q.keys()[0]].write(format=9)) 
        outputfile.close()
